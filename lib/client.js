window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*
		* 设计（跟随 DSH 官方视觉语言：克制极简、扁平纯色、--dsw-alias-* token、小圆角 + 胶囊 + 状态小点）：
		*
...
.dshu .title{font-size:14px;font-weight:600;line-height:1.4;color:var(--ink)}
.dshu .desc{font-size:13px;color:var(--muted);line-height:1.65;margin-top:4px}
...
.dshu .pill{font-size:12px;font-weight:600;font-variant-numeric:tabular-nums;letter-spacing:.2px;padding:4px 10px;border-radius:999px;background:var(--dsw-alias-bg-module-platform,rgba(0,0,0,.04));border:1px solid var(--bd);color:var(--muted);white-space:nowrap}
...
.dshu .logsum{display:flex;align-items:center;gap:6px;width:100%;padding:7px 10px;border:0;background:none;cursor:pointer;font-size:12px;color:var(--muted);text-align:left}
...
.dshu .log{font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:11px;color:var(--faint);max-height:110px;overflow-y:auto;white-space:pre-wrap;word-break:break-all;padding:8px 10px;border-top:1px solid var(--bd)}
...
.dshu .link{background:none;border:0;color:var(--accent);cursor:pointer;font-size:13px;padding:4px 2px;text-decoration:none}
.dshu .link:hover{text-decoration:underline}
.dshu .countdown{display:inline-flex;align-items:center;font-size:13px;font-weight:700;font-variant-numeric:tabular-nums;color:var(--accent)}
/* ── 侧边栏底部入口 ─────────────────────────────── */
...
`;
		...
		const checkBus = {
			...
			set(v, status) {
				this.checking = v;
				for (const fn of this.listeners) fn(v, status);
			},
			subscribe(fn) {
				this.listeners.add(fn);
				return () => this.listeners.delete(fn);
		...
		function UpdateWidget() {
			...
			const recheck = React.useCallback(async () => {
				...
				let result;
				try {
					result = await api("/dsh-update/check");
					setS(result);
				} catch (_e) {
					refresh();
				} finally {
					setChecking(false);
					checkBus.set(false, result);
				}
			}, [refresh]);
			React.useEffect(() => {
				refresh();
				const refreshMs = s && (s.status === "installing" || s.status === "installed" || s.status === "restarting") ? 1e3 : 15e3;
		...
		function UpdateEntry({ wide }) {
			const [s, setS] = React.useState(null);
			const [checking, setChecking] = React.useState(checkBus.checking);
			const refresh = React.useCallback(async () => {
				try {
					const st = await api("/dsh-update/status");
					setS(st);
			...
			React.useEffect(() => checkBus.subscribe((active, result) => {
				setChecking(active);
				// 检查完成带结果 → 直接采用；异常/无结果（result 为空）→ 立即拉一次真实状态，
				// 避免入口停留在旧的对勾、看起来像"没检查完就恢复"
			...
			if (!wide) return null;
			const st = s.status;
			// busy 需包含 host 的 checking：手动检查走 checkBus，但启动/定时自动检查
			// 不会通知 checkBus，只能靠 status 判断 —— 漏掉会显示对勾而非 spinner
			const busy = checking || st === "checking" || st === "installing" || st === "restarting";
