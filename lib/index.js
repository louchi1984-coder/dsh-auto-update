...
function currentVersion() {
  try {
    ...
    // npm 全局安装的常见 Unix 布局：<node-prefix>/lib/node_modules/@deepseek-ai/dsh。
...
// 只清理无法跨进程恢复的瞬态状态；已确认的版本信息和“稍后”记录应在首次扫描前可见。
if (["checking", "installing", "installed", "restarting"].includes(state.status)) {
  state.status = "idle";
  ...
  ctx.effect(() =>
    ctx.webServer.register({
      ...
      handler: async (req, res) => {
        // 共享同一个检查 promise：不论是首次请求还是并发请求，都等到最终状态再返回。
        try {
          await checkNow();
